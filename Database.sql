...
INSERT INTO powers (name, description)
VALUES ('Teleportation', 'The transfer of matter from one point to another without traversing the physical space between them.');
...
SELECT AVG(power_level) AS "Average Super Strength Power Level" FROM heroes_powers
WHERE power_id = 4;
...
SELECT powers.id, powers.name, powers.description, AVG(power_level) FROM heroes_powers
JOIN powers ON heroes_powers.power_id = powers.id
GROUP BY powers.id, powers.name, powers.description;
